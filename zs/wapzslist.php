...
<body>
	...
	if($keyword == ""){		
		...
		echo "<script language='javascript'>layer.msg('查询条件不能为空！',{icon:0,time: 3000,shift: -1}, function(){location.href='../wapag.php';});</script>";
		exit();
	}
	
	if($cf['yzm_status'] == 1 && $yzm != $_SESSION['authnum_session'])
	{
		echo "<script language='javascript'>layer.msg('验证码错误！',{icon:0,time: 3000,shift: -1}, function(){location.href='../wapag.php';});</script>";	
		exit();
	}    
...
if($shzt == "2"){
	echo "<script language='javascript'>layer.msg('该代理还在审核中！',{icon:0,time: 3000,shift: -1}, function(){location.href='../wapag.php';});</script>";
	exit;
	
}	
if($hmd == "1"){
	echo "<script language='javascript'>layer.msg('该代理已被列为黑名单！',{icon:0,time: 3000,shift: -1}, function(){location.href='../wapag.php';});</script>";
	exit;
	
}	

if( $jietime < $dqts){
	echo "<script language='javascript'>layer.msg('对不起，该代理授权到期，请联系我们续签代理！',{icon:0,time: 3000,shift: -1}, function(){location.href='../wapag.php';});</script>";
	exit;
}   
$results     = "4";
...
{
	$results = "6";	 
	echo "<script language='javascript'>layer.msg('没有找到相关的代理商信息！',{icon:0,time: 3000,shift: -1}, function(){location.href='../wapag.php';});</script>";
...
<div class="banner"><h1>证书已直接生成图片，长按可保存！</h1> </div>
