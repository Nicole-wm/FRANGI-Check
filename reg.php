...
require("themes/".$cf['agent_themes']."/reg.html");
}else{
...
if($act == "regagent")
{
   ...
   if($agentid=="")
	...
	echo "<script language='javascript'>layer.msg('代理商编号不能为空!', function(){});</script>";
	...
	if($name=="")
	{
	echo "<script language='javascript'>layer.msg('姓名不能为空!', function(){});</script>";

	  exit;
	}
	if($phone=="")
	{
	echo "<script language='javascript'>layer.msg('手机号码不能为空!', function(){});</script>";

	  exit;
	}
	if (strlen($phone) != "11") {
 
   echo "<script language='javascript'>layer.msg('手机号码格式不对!', function(){});</script>";

	  exit;
   }
	if($weixin=="")
	{
	echo "<script language='javascript'>layer.msg('微信号不能为空!', function(){});</script>";

	  exit;
	}
	if($idcard=="")
	{
	echo "<script language='javascript'>layer.msg('身份证号不能为空!', function(){});</script>";

	  exit;
	}
	if($quyu=="")
	{
	echo "<script language='javascript'>layer.msg('期待代理区域不能为空!', function(){});</script>";

	  exit;
	}
	if($cf['yzm_status'] == 1 && $yzm != $_SESSION['authnum_session'])
		{
	echo "<script language='javascript'>layer.msg('验证码错误!', function(){});</script>";
...

	{
	 ...
	 echo "<script language='javascript'>layer.msg('上级代理微信号不存在!', function(){});</script>";
	...
	if(mysql_num_rows($res)>0){
	 ...
	 echo "<script language='javascript'>layer.msg('微信号已存在!', function(){});</script>";
	...
	echo "<script language='javascript'>layer.msg('手机号已存在!', function(){});</script>";
	...
	mysql_query($sql);
	echo "<script language='javascript'>layer.msg('代理申请提交成功，请等待审核。<br>审核通过后，我们会短信通知您！',{icon:1,time: 5000,shift: -1});</script>";
	///短信通知开始，调用阿里大于短信
